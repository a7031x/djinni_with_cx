// AUTOGENERATED FILE - DO NOT MODIFY!
// This file generated by Djinni from derivings.djinni

#import "RecordWithNestedDerivings.h"


@implementation RecordWithNestedDerivings

- (nonnull instancetype)initWithKey:(int32_t)key
                                rec:(nonnull RecordWithDerivings *)rec
{
    if (self = [super init]) {
        _key = key;
        _rec = rec;
    }
    return self;
}

+ (nonnull instancetype)recordWithNestedDerivingsWithKey:(int32_t)key
                                                     rec:(nonnull RecordWithDerivings *)rec
{
    return [[self alloc] initWithKey:key
                                 rec:rec];
}

- (BOOL)isEqual:(id)other
{
    if (![other isKindOfClass:[RecordWithNestedDerivings class]]) {
        return NO;
    }
    RecordWithNestedDerivings *typedOther = (RecordWithNestedDerivings *)other;
    return self.key == typedOther.key &&
            [self.rec isEqual:typedOther.rec];
}

- (NSUInteger)hash
{
    return NSStringFromClass([self class]).hash ^
            (NSUInteger)self.key ^
            self.rec.hash;
}

- (NSComparisonResult)compare:(RecordWithNestedDerivings *)other
{
    NSComparisonResult tempResult;
    if (self.key < other.key) {
        tempResult = NSOrderedAscending;
    } else if (self.key > other.key) {
        tempResult = NSOrderedDescending;
    } else {
        tempResult = NSOrderedSame;
    }
    if (tempResult != NSOrderedSame) {
        return tempResult;
    }
    tempResult = [self.rec compare:other.rec];
    if (tempResult != NSOrderedSame) {
        return tempResult;
    }
    return NSOrderedSame;
}

@end
